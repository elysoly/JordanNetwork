function  [selected_a,selected_ar,selected_b,selected_bout,selected_z,selected_y]=jordan_net_batch(X,val_data,ts_data,epsilon1,epsilon2,hidden_neuron,handles)
% clear;
% close all;
% clc;
% 
...
batch_size=str2num(handles.batch_size.String);
hidden_neuron=2;
...
b=ones(hidden_neuron,2).*rand(hidden_neuron,2);%b
bias_hidden=zeros(hidden_neuron,1);
bias_out=zeros(hidden_neuron,1);

y=ones(hidden_neuron,1).*rand(hidden_neuron,1);%y
z(1)=X(1);
...
while(epoch<=threshold)
    for t=1:length(X)-1
        for i=1:hidden_neuron
            index=fix((t-1)/batch_size)+1;
            U(i)=a(i,index)*X(t) + a_r(i,index)*z(t); 
            y(i,t+1)=f_h(U(i));
        end
        v(t+1)=sum(b(:,index+1).*y(:,t+1)) + bias_out(i,index);
        z(t+1)=f_out(v(t+1));
        ...
        for i=1:hidden_neuron
            Q(i,t+1)=z(t+1)*(1-z(t+1))*b(i,index+1)*y(i,t+1)*(1-y(i,t+1));
            phi(i,t+1)=Q(i,t+1)*(z(t)+ a_r(i,index)*phi(i,t));
            delta_b(i,t+1) = -epsilon1*p(t+1)*y(i,t+1);
            delta_a(i,t)=-epsilon2*p(t+1)* b(i,index+1)*f_prime_h(U(i))*X(t);
             if(isnan(delta_a(i,t)))
                fprintf('yes');
            end
            delta_ar(i,t)=-epsilon2*(z(t+1)-X(t+1))*phi(i,t+1);
            delta_bias_out(t)=-epsilon1*p(t+1);
        end

        if(mod(t,batch_size)==0 || t>=length(X))
        %%% do update
            batch_start=(fix((t-1)/batch_size))*batch_size +1;
            deltabar_b(:,index)=sum( delta_b(:,batch_start:t+1),2);
            deltabar_a(:,index)=sum(delta_a(:,batch_start:t),2);
            deltabar_ar(:,index)=sum(delta_ar(:,batch_start:t),2);
            deltabar_bias_out(:,index)=sum(delta_bias_out(batch_start:t),2);
            b(:,index+2)=b(:,index+1)+deltabar_b(:,index);
            a(:,index+1)=a(:,index)+deltabar_a(:,index);
             if(isnan(a(:,index+1)))
                fprintf('yes');
            end
            a_r(:,index+1)=a_r(:,index)+deltabar_ar(:,index);
    %         bias_hidden(:,t+1)=-epsilon2*p(t+1)* b(i,t+1)*f_prime_h(U(i));
            bias_out(:,index+1)=bias_out(:,index)+deltabar_bias_out(:,index);
        end
    end
    z=z.';
    switch measure_type
